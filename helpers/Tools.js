...
*/
  changeUrl(urlLabel) {
    ...
    let url_type = "";
    let url_index = "";
    if (urlLabel && urlLabel !== "") {
      if (urlLabel.indexOf(":")) {
        temp = urlLabel.split(":");//转换成数组
        url_type = temp[0];//：前面的
        url_index = temp[1];//：后面的
      } else {
        url_type = urlLabel;//不存在：就直接拿出来
      }
    }
    var typeArr = ["url"];
    //$.inArray(value,array)    --value是要查找的值，array是被查找的数组。
    if ($.inArray(url_type, typeArr) === -1) {
      data[0] = urlLabel;
      return data;
    } else {
      if (url_type === 'url') {
        return urlLabel.split("url:")[1] ? urlLabel.split("url:")[1] : "";
      }
    }
    return url
  }
